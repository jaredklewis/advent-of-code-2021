...
function chunk<T>(array: T[], perChunk: number) {
  return array.reduce<Array<Array<T>>>((chunkedArray, item, index) => {
...
type Board = Array<Array<number | "x">>;
...
function parse(text: string): Input {
  ...
  if (!firstLine) {
    throw new Error("Invalid call line");
  }

  const calls = firstLine.split(",").map((call) => parseInt(call));

  const boards = lines
    .slice(2)
    .map((line) => line.trim())
    .filter((line) => !!line)
    .map((line) => line.split(/\s+/).map((cell) => parseInt(cell)));

  return { calls, boards: chunk(boards, 5) };
}

function findWinner({ boards, calls }: Input, findLast = false): Result {
  let myBoards = boards.slice(0);
  ...
  for (const call of calls) {
    myBoards = myBoards.map((board) =>
      board.map((row) =>
        row.map((cell) => {
          return call === cell ? "x" : cell;
        })
      )
    );

    for (let i = 0; i < myBoards.length; i++) {
      const board = myBoards[i]!!;

      for (let j = 0; j < board.length; j++) {
        const row = board[j]!!;
        if (row.filter((cell) => cell === "x").length === 5) {
          if (findLast) {
            winners.add(i);
            if (winners.size === myBoards.length) {
              return { board, call };
            }
          } else {
            return { board, call };
...
function calculateScore({ board, call }: Result) {
  const sum = board.reduce((total, row) => {
    return (
      total +
      row.reduce<number>((subTotal, cell) => {
        if (cell === "x") {
          return subTotal;
        }
...
const exampleWinner = findWinner(parse(EXAMPLE));
assert.equal(4512, calculateScore(exampleWinner));

const input = parse(fs.readFileSync("input.txt", { encoding: "utf-8" }));
const winner = findWinner(input);

console.log("Part one - first winner score", calculateScore(winner));
...
const exampleLoser = findWinner(parse(EXAMPLE), true);
assert.equal(1924, calculateScore(exampleLoser));
const loser = findWinner(input, true);

console.log("Part two - first last score", calculateScore(loser));
